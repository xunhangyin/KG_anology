...
class ModelTrainer(object):
    def __init__(self, pretrained_clip, output_dir, learning_rate, batch_size, k_shot, num_epochs=5,
        ...
        self.pretrained_clip = pretrained_clip   ###huggingface CLIP预训练初始权重路径
        self.output_dir = output_dir         ###微调后权重保存路径
        self.lr = learning_rate
        self.batch_size = batch_size
        self.num_epochs = num_epochs
        self.accu_steps = accumulate_steps    ###没有梯度累计，放这了
        self.checkpoint_steps = checkpoint_steps     ###没用
        self.tempeature = tempature      ###没用
        self.weight_decay = 1e-4
        self.k_shot = k_shot      ###这个解释一下，就是t-h=a-q,a=q+t-h，这里q+t-h是正样本，其与a的距离做softmax的分子以及分母.k_shot就是对比学习softmax中与a的距离做分母的对比样本，越多的话任务越难一些

    def divide_sample(self, dataset):   ###对json字典数据做一下初步处理，分出q,t,h,为data,a 为sample
        data = []
        label = []
    ...
    def get_pil_image(self, image_path):    ###给定路径获取其中的图片，以PIL形式读取
    ...
    def get_image(self, image_path, data_batch, label_batch):        ###对于每个batch的sample,对q,t,h,a进行图片提取
        image_example_heads = []
        image_example_tails = []
        image_ques = []
        image_answers = []
        ...
        return image_example_heads, image_example_tails, image_ques, image_answers    ###依次返回，都是batch_images

    def get_text(self, text_path, data_batch, label_batch, id2text, id2des):     ##同样，获取文本
        text_example_heads = []
        text_example_tails = []
        text_ques = []
        text_answers = []
    ...
    def image_to_emb(self, batch_image, image_encoder, image_processor):    ###对于一个batch的图片，比如batch为5，每个有4到6张图片，最后返回5*512的tensor
    ...
    def text_to_emb(self, batch_text, text_encoder, tokenizer):    ###同样，类似于image_to_emb
    ...
    def get_entity_image_emb(self, k_samples, image_path, image_encoder, image_processor):      ###相反，这个是给定k_shot个对比实体，获得他们图像的embedding
        sample_images = []
        sample_texts = []
    ...
    def get_entity_text_emb(self, k_samples, id2text, id2des, text_encoder, tokenizer):     ###类似于上面，获得对比实体的文字embedding
        sample_texts = []
        for sample in k_samples:
            name = id2text[sample]
            des = id2des[sample]
    ...
    def get_entities_emb(self, batch_k_samples, image_path, id2text, id2des, image_encoder, text_encoder,
                         image_processor, tokenizer, pin_model):     ###获取图像和文字emb后结合起来获取每个实体的emb
        num = 0
    ...
    def compute_loss(self, batch_ans_pred, batch_ans_embed, batch_k_embeds):   ###loss计算
        fenzi = 0
        fenmu = 0
        loss = 0
    ...
    def get_data(self, batch_idx, total_samples, train_data, train_label):    ###读取数据的函数
    ...
    def train(self, dataset_train_json, entity_des_dict, rel_des_dict, entities, dataset_val_json=None,
              dataset_test_json=None):     ###训练的主函数了
        train_data, train_label = self.divide_sample(dataset_train_json)

        pin_model = concat_model_linear(512)                      ###这个模型在concat_model.py里定义，输入文本嵌入和图像嵌入，进过linear和激活函数生成一个embedding代表实体
        ...
        pin_model=nn.DataParallel(pin_model,device_ids=[0,1,2,3])
        image_encoder=nn.DataParallel(image_encoder,device_ids=[0,1,2,3])
        ...
        for epoch in range(self.num_epochs):
            for batch_idx in range(math.ceil(len(train_data) / self.batch_size)):
                data_batch=train_data[batch_idx]
                ...
                if batch_idx % 1000 ==0 and batch_idx != 0:
                    torch.save(image_encoder,"./CLIP_model2/"+str(epoch)+"_"+str(batch_idx)+"_image_encoder.pt")
                    torch.save(text_encoder,"./CLIP_model2/"+str(epoch)+"_"+str(batch_idx)+"_text_encoder.pt")
                    torch.save(pin_model,"./CLIP_model2/"+str(epoch)+"_"+str(batch_idx)+"_pin_model.pt")
                    print("model saved")
